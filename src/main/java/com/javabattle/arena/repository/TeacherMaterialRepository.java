...
public interface TeacherMaterialRepository extends JpaRepository<TeacherMaterial, Long> {
    ...
    @Query("SELECT tm FROM TeacherMaterial tm WHERE tm.parentMaterialId = :parentId OR tm.id = :id ORDER BY tm.createdAt DESC")
    List<TeacherMaterial> findByParentMaterialIdOrId(@Param("parentId") Long parentId, @Param("id") Long id);
    
    int countByParentMaterialId(Long parentMaterialId);
    
    List<TeacherMaterial> findByParentMaterialId(Long parentMaterialId);
    
    @Query("SELECT tm FROM TeacherMaterial tm WHERE tm.isLatestVersion = true ORDER BY tm.createdAt DESC")
    List<TeacherMaterial> findLatestVersions();
}